...
class Classifier_Module(nn.Layer):
    ...
    def __init__(self, dims_in, dilation_series, padding_series, num_classes):
        ...
        for dilation, padding in zip(dilation_series, padding_series):
            self.conv2d_list.append(nn.Conv2D(dims_in, num_classes, kernel_size=3, stride=1, padding=padding, dilation=dilation))
...
class DeeplabVGG(nn.Layer):
    def __init__(self, num_classes, vgg16_caffe_path=None, pretrained=False):
        ...
        self.features = nn.Sequential(*([features[i] for i in range(len(features))] + [ fc6, nn.ReLU(), fc7, nn.ReLU()]))
